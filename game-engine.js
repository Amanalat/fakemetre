...
let cursorTutorialSeen=false; // Ne montrer le tutoriel curseur qu'une fois

// Historique détaillé pour le bilan personnalisé
let gameHistory = [];
...
function chooseLevel(level){
  ...
  if(level==='junior'){
    $('modescreen-title').textContent = '🔎 Apprenti Détective — Choisis ton mode';
  } else if(level==='inter'){
    $('modescreen-title').textContent = '🕵️ Enquêteur Junior — Choisis ton mode';
  } else {
    $('modescreen-title').textContent = '⚡ Fact-Checker Confirmé — Choisis ton mode';
  }
}
...
function backToMenu(){
  if(confirm('⚠️ Quitter la partie en cours ? Ta progression sera perdue.')){
...
function startGame(mode){
  ...
  gameHistory=[]; // Réinitialiser l'historique pour le bilan
  $('modescreen').style.display='none';
...
function restart(){
  $('final').style.display='none';
  gameHistory=[]; // Réinitialiser l'historique
  if(playingCustom){
    // Rejouer les questions custom
...
function updMiniMeter(p){
  ...
  if(p<33){$('mini-mfill').style.background='linear-gradient(90deg,#f75f5f,#f7a548)';s.textContent='⬅️ Plutôt FAUX';s.style.color='#f75f5f';}
  else if(p>66){$('mini-mfill').style.background='linear-gradient(90deg,#3ecf8e,#4fa3f7)';s.textContent='Plutôt VRAI ➡️';s.style.color='#3ecf8e';}
  else{$('mini-mfill').style.background='linear-gradient(90deg,#f7c948,#4fa3f7)';s.textContent='🤔 Incertain...';s.style.color='#f7c948';}
}
...
function updMeter(){
  ...
  const s=$('mstatus');
  if(p<33){$('mfill').style.background='linear-gradient(90deg,#f75f5f,#f7a548)';s.textContent='⬅️ Plutôt FAUX';s.style.color='#f75f5f';}
  else if(p>66){$('mfill').style.background='linear-gradient(90deg,#3ecf8e,#4fa3f7)';s.textContent='Plutôt VRAI ➡️';s.style.color='#3ecf8e';}
  else{$('mfill').style.background='linear-gradient(90deg,#f7c948,#4fa3f7)';s.textContent='🤔 Incertain...';s.style.color='#f7c948';}
}
...
function updCounter(){
  ...
  if(uc===0) $('counter').textContent=`Consulte au moins 1 source avant de voter (${total} disponibles)`;
  else if(remaining>0) $('counter').textContent=`✅ Tu peux voter ou consulter encore ${remaining} source${remaining>1?'s':''} !`;
  else $('counter').textContent=`✅ Tu as évalué toutes les sources. À toi de décider !`;
}
...
function renderQ(){
  ...
  // Afficher le bouton "Pas de consensus" sur TOUTES les questions en niveau pro
  const btnConsensus = $('btn-consensus');
  if(btnConsensus){
    btnConsensus.style.display = (currentLevel==='pro') ? 'block' : 'none';
  }
  // Montrer le tutoriel curseur seulement si pas encore vu ET première question de la partie
...
let likertVal=0;
const LIKERT_LABELS=['','Absolument pas fiable','Très peu fiable','Peu fiable','Neutre — difficile à dire','Plutôt fiable','Très fiable','Totalement fiable'];
...
function confirmLikert(){
  ...
  // Mettre à jour le tag (créer ou recycler)
  ...
  $('fbtitle').textContent=isEdit?'Note modifiée !':'Source évaluée !';
  $('fbexpl').textContent='Tu as noté cette source '+likertVal+'/7. Tu peux consulter d\'autres sources ou voter directement.';
  $('fbpts').textContent='';$('fbpts').className='fbpts';
  // Swap atomique : montrer mfb avant de cacher msrc pour éviter le gap sans overlay
  show('mfb');
...
function searchSource(){
  // Ouvrir une recherche Google sur la source actuelle
  if(!pend) return;
  const src = pend.src;
  
  // Si la source a une URL directe, l'ouvrir
  if(src.url) {
    window.open(src.url, '_blank');
    return;
  }
  
  // Si la source est fictive/générique, afficher un message
  if(src.noSearch) {
    alert("🔍 Impossible de trouver cette source sur internet !\n\nC'est une source fictive ou trop générique pour être vérifiée. Dans la vraie vie, c'est un signal d'alerte : une source introuvable est suspecte !");
    return;
  }
...
function trust(trusts){
  ...
  if(src.g===true&&trusts)  {oc='ug';emoji='🎉';title='Excellent choix !';expl=src.fb.tg||'';}
  else if(src.g===true)     {oc='ub';emoji='😬';title='Dommage...';       expl=src.fb.dg||'';}
  else if(trusts)            {oc='ub';emoji='😬';title='Attention !';      expl=src.fb.tb||'';}
  else                       {oc='ug';emoji='👏';title='Bien joué !';      expl=src.fb.db||'';}
  if(!isEdit) uc++;
  um[idx]=oc;
  btn.classList.remove('ug','ub');
  ...
  tag.textContent=trusts?'✅ Crue':'🙅 Ignorée';
  updCounter();
...
function submitV(v){
  ...
  if(v === 'nuance' && q.nuance){
    ok = true;
    emoji = '🧠';
    title = "Excellent sens critique !";
    expl = q.enuan || q.e;
  } else if(v === 'nuance' && !q.nuance){
    ok = false;
    emoji = '🤔';
    title = "Hmm, ici il y a bien un consensus...";
    expl = q.e;
  } else {
    ok = v === q.t;
    emoji = ok ? '🎉' : '😬';
    title = ok ? (q.t ? "✅ Oui, c'est bien VRAI !" : "✅ Oui, c'est bien FAUX !") : (q.t ? "❌ Non, c'était VRAI !" : "❌ Non, c'était FAUX !");
    expl = q.e;
  }
  ...
  // Récupérer les évaluations de sources
  sh.forEach((src, i) => {
    if(um[i] !== undefined){
...
function showFinal(){
  ...
  let m,c;
  if(pct>=.8){m='🥇';c="Incroyable ! Tu es un vrai détective de l'info !";}
  else if(pct>=.5){m='🥈';c="Bien joué ! Tu as de bons réflexes de vérification !";}
  else if(pct>=.25){m='🥉';c="Pas mal ! Continue à t'entraîner à vérifier les infos !";}
  else{m='📚';c="Les fake news t'ont bien eu... mais maintenant tu sais comment réagir !";}
  
  // Générer le bilan personnalisé
  const bilan = generateBilan();
  
  const tips=[['✅','Consulte un expert ou spécialiste du sujet'],['✅','Utilise des sources officielles (.gouv, agences scientifiques)'],['✅','Vérifie sur un site de fact-checking (Hoaxbuster, AFP Factuel…)'],['✅','Lis des encyclopédies ou livres scientifiques'],['❌','Les likes et partages ne prouvent rien'],['❌','"Tout le monde le dit" ne prouve rien'],['❌','Une info drôle ou choquante doit être encore plus vérifiée']];
  $('final').style.display='block';
  $('final').innerHTML=`
    <span class="fmedal">${m}</span>
    <h2 class="fh2">Enquête terminée !</h2>
    ...
    <div class="ftips"><h3>📋 Les bons réflexes à retenir</h3>${tips.map(t=>`<div class="tip"><span>${t[0]}</span><span>${t[1]}</span></div>`).join('')}</div>
    <button id="rbtn" onclick="restart()">🔄 Rejouer</button>
...
function generateBilan(){
  ...
  let absoluteTrust = []; // Sources notées 7/7
  let wrongVotes = [];
  let missedNuances = [];
  
  gameHistory.forEach(h => {
    // Analyser les votes VRAI/FAUX
    if(!h.wasCorrect){
    ...
    // Analyser les évaluations de sources
  ...
  if(trustedBadSources.length > 0){
    insights.push(`<div class="bilan-item bad">
      ...
      <div>
        <strong>Tu as fait confiance à des sources peu fiables :</strong><br>
        <em>${trustedBadSources.slice(0,3).join(', ')}</em><br>
        <span class="bilan-tip">→ Méfie-toi des sources qui ont l'air sérieuses mais qui ne citent pas leurs propres sources, ou qui ont un intérêt à te convaincre.</span>
      </div>
    </div>`);
  }
  ...
  if(distrustedGoodSources.length > 0){
    insights.push(`<div class="bilan-item warn">
      ...
      <div>
        <strong>Tu t'es méfié(e) de sources pourtant fiables :</strong><br>
        <em>${distrustedGoodSources.slice(0,3).join(', ')}</em><br>
        <span class="bilan-tip">→ Les sources institutionnelles, scientifiques ou de fact-checking sont généralement fiables, même si elles semblent moins "fun" que d'autres.</span>
      </div>
    </div>`);
  }
  ...
  if(absoluteTrust.length > 0){
    insights.push(`<div class="bilan-item info">
      <span class="bilan-icon">💡</span>
      <div>
        <strong>Tu as noté 7/7 (confiance absolue) à certaines sources</strong><br>
        <span class="bilan-tip">→ Même les meilleures sources peuvent se tromper ! Garde toujours un petit doute et croise plusieurs sources fiables.</span>
      </div>
    </div>`);
  }
  ...
  if(missedNuances.length > 0){
    insights.push(`<div class="bilan-item info">
      <span class="bilan-icon">⚖️</span>
      <div>
        <strong>Tu as manqué des nuances :</strong><br>
        <em>${missedNuances.slice(0,2).join(', ')}</em><br>
        <span class="bilan-tip">→ Certains sujets n'ont pas de réponse tranchée VRAI/FAUX. Repérer les débats scientifiques, c'est aussi du fact-checking !</span>
      </div>
    </div>`);
  }
  ...
  if(wrongVotes.length > 0 && insights.length < 3){
    insights.push(`<div class="bilan-item bad">
      <span class="bilan-icon">❌</span>
      <div>
        <strong>Erreurs sur le verdict final :</strong><br>
        <span class="bilan-tip">→ Continue à consulter les sources avant de voter. Plus tu en consultes, plus tu affines ton jugement !</span>
      </div>
    </div>`);
  }
  ...
  if(insights.length === 0){
    insights.push(`<div class="bilan-item good">
      ...
      <div>
        ...
        <span class="bilan-tip">Tu as bien évalué les sources et voté correctement. Continue comme ça !</span>
      </div>
    </div>`);
  }
  
  return `<h3 style="margin-bottom:12px;">📊 Ton bilan personnalisé</h3>${insights.join('')}`;
}
...
function updateQuestionCount(){
  ...
  // La section import est toujours visible (gérée directement dans le HTML)
}
...
function renderSources(){
  ...
  if(currentSources.length === 0){
    container.innerHTML = '<p style="color:var(--mu);font-size:.85rem;text-align:center;padding:20px;">Clique sur "Ajouter une source" pour commencer.</p>';
    return;
  }
  container.innerHTML = currentSources.map((src, i) => {
    const typeKey = srcTypeKey(src.g);
    const typeInfo = {
      reliable:   {icon:'✅', label:'Fiable',           desc:'Le joueur doit lui faire confiance'},
      unreliable: {icon:'❌', label:'Non fiable',        desc:'Le joueur ne doit pas lui faire confiance'},
      free:       {icon:'🎚️', label:'Joueur évalue',    desc:'Le joueur utilise l\'échelle de Likert — pas de bonne réponse imposée'},
    }[typeKey];
    ...
    if(typeKey === 'free'){
      feedbackHTML = `
        <div class="form-group">
          <label>💬 Feedback général après évaluation (optionnel)</label>
          <input type="text" value="${src.fb.gen||''}" onchange="updateSourceFb(${src.id},'gen',this.value)" placeholder="Ex: Cette source est complexe — voici pourquoi...">
        </div>`;
    } else if(typeKey === 'reliable'){
      feedbackHTML = `
        <div class="form-group">
          <label>✅ Si le joueur lui fait confiance (bonne réponse)</label>
          <input type="text" value="${src.fb.tg||''}" onchange="updateSourceFb(${src.id},'tg',this.value)" placeholder="Bravo ! Cette source est fiable car...">
        </div>
        <div class="form-group">
          <label>❌ Si le joueur l'ignore (mauvaise réponse)</label>
          <input type="text" value="${src.fb.dg||''}" onchange="updateSourceFb(${src.id},'dg',this.value)" placeholder="Dommage ! Cette source était fiable car...">
        </div>`;
    } else {
      feedbackHTML = `
        <div class="form-group">
          <label>❌ Si le joueur lui fait confiance (mauvaise réponse)</label>
          <input type="text" value="${src.fb.tb||''}" onchange="updateSourceFb(${src.id},'tb',this.value)" placeholder="Attention ! Cette source n'est pas fiable car...">
        </div>
        <div class="form-group">
          <label>✅ Si le joueur l'ignore (bonne réponse)</label>
          <input type="text" value="${src.fb.db||''}" onchange="updateSourceFb(${src.id},'db',this.value)" placeholder="Bien joué ! Tu as bien fait de te méfier car...">
        </div>`;
    }
    ...
    <div class="source-card ${typeKey}" id="src-${src.id}">
      ...
      <label style="display:block;font-weight:700;font-size:.8rem;color:var(--mu);margin-bottom:6px;">Type de source</label>
      <div class="stype-btns">
        ...
        <button class="stype-btn ${typeKey==='unreliable'?'active-unreliable':''}" onclick="setSourceType(${src.id},'unreliable')">❌ Non fiable</button>
        <button class="stype-btn ${typeKey==='free'?'active-free':''}" onclick="setSourceType(${src.id},'free')">🎚️ Joueur évalue</button>
      </div>
      ...
      <div class="form-group" style="margin-bottom:8px;">
        ...
        <div style="display:flex;gap:8px;align-items:center;">
          ...
          <button type="button" onclick="triggerFileUpload(${src.id})" class="creator-btn secondary" style="padding:8px 14px;font-size:.8rem;white-space:nowrap;flex-shrink:0;" ${src.noSearch?'disabled':''}>📎 Fichier</button>
        ...
        ${src.url && src.url.match(/\.pdf(\?|$)/i) ? `<div style="margin-top:6px;font-size:.78rem;color:var(--bl);font-weight:700;">📄 PDF joint — <a href="${escH(src.url)}" target="_blank" style="color:var(--bl);">Aperçu</a></div>` : ''}
      </div>
      <div class="nosearch-row" style="margin-bottom:12px;">
        ...
        <label for="nosearch-${src.id}">🚫 Source fictive / introuvable sur internet</label>
      </div>
...
function resetForm(){
  editingIndex = -1;
  $('edit-title').textContent = '➕ Nouvelle question';
  $('c-statement').value = '';
  $('c-explanation').value = '';
  $('c-enuan').value = '';
  currentNuance = false;
...
function validateQuestion(){
  ...
  if(!statement){
    alert('⚠️ Entre une affirmation à vérifier !');
    return false;
  }
  if(!explanation){
    alert('⚠️ Entre une explication !');
    return false;
  }
  if(currentSources.length < 2){
    alert('⚠️ Ajoute au moins 2 sources !');
    return false;
  }
  ...
  for(let i=0; i<currentSources.length; i++){
    if(!currentSources[i].n.trim() || !currentSources[i].say.trim()){
      alert(`⚠️ La source ${i+1} doit avoir un nom et une citation !`);
      return false;
    }
  }
  return true;
  ...
  const question = {
    ...
    src: currentSources.map(s => {
      ...
      const srcObj = {
        ...
        g: s.g,
        fb: {
          tg: s.fb.tg || (tk==='reliable' ? 'Bonne source ! Tu as bien fait de lui faire confiance.' : ''),
          dg: s.fb.dg || (tk==='reliable' ? 'Cette source était pourtant fiable.' : ''),
          tb: s.fb.tb || (tk==='unreliable'||tk==='weak' ? 'Cette source n\'était pas fiable !' : ''),
          db: s.fb.db || (tk==='unreliable'||tk==='weak' ? 'Bien joué de t\'en méfier !' : ''),
          gen: s.fb.gen || (tk==='free' ? 'Source évaluée ! Tu peux consulter d\'autres sources ou voter.' : '')
        }
      };
...
function renderQuestionsList(){
  ...
  if(customQuestions.length === 0){
    container.innerHTML = `<div class="empty-state">
      ...
      <p>Aucune question créée pour l'instant.<br>Commence par en créer une !</p>
    </div>`;
    return;
  }
  container.innerHTML = customQuestions.map((q, i) => `
    <div class="question-card" onclick="editQuestion(${i})">
      ...
      <div class="q-meta">
        <span class="q-badge ${q.t ? 'true' : 'false'}">${q.t ? 'VRAI' : 'FAUX'}</span>
        ${q.nuance ? '<span class="q-badge" style="background:rgba(247,201,72,.2);color:#f7c948;">⚖️ NUANCÉ</span>' : ''}
        <span>📚 ${q.src.length} sources</span>
        <span style="margin-left:auto;">✏️ Modifier</span>
      </div>
    </div>
  `).join('') + `
    <div class="creator-actions" style="margin-top:16px;">
      <button class="creator-btn danger" onclick="clearAllQuestions()">🗑️ Tout supprimer</button>
    </div>
  `;
}
...
function editQuestion(index){
  ...
  editingIndex = index;
  $('edit-title').textContent = '✏️ Modifier la question';
...
function clearAllQuestions(){
  if(confirm('⚠️ Supprimer TOUTES les questions ? Cette action est irréversible !')){
    customQuestions = [];
    saveToStorage();
    updateQuestionCount();
    renderQuestionsList();
  ...
  reader.onload = function(e){
    try {
      ...
      if(Array.isArray(imported)){
        if(confirm(`Importer ${imported.length} question(s) ? (Les questions existantes seront conservées)`)){
...
function exportCSV(){
  ...
  let csv = 'Affirmation;Vrai/Faux;Explication;Nuance;ExplicationNuance;Source1_Emoji;Source1_Nom;Source1_Description;Source1_Citation;Source1_Fiable;Source1_URL;Source1_Fictive;Source1_FB_Positif;Source1_FB_Negatif;Source2_Emoji;Source2_Nom;Source2_Description;Source2_Citation;Source2_Fiable;Source2_URL;Source2_Fictive;Source2_FB_Positif;Source2_FB_Negatif;Source3_Emoji;Source3_Nom;Source3_Description;Source3_Citation;Source3_Fiable;Source3_URL;Source3_Fictive;Source3_FB_Positif;Source3_FB_Negatif;Source4_Emoji;Source4_Nom;Source4_Description;Source4_Citation;Source4_Fiable;Source4_URL;Source4_Fictive;Source4_FB_Positif;Source4_FB_Negatif\n';
  ...
  reader.onload = function(e){
    try {
        ...
        for(let j=0; j<4; j++){
          ...
          if(cols[base+1] && cols[base+1].trim()){ // If source name exists
            ...
            if(isNewFormat){
              ...
              if(isReliable){ src.fb.tg = fbPos || 'Bonne source !'; src.fb.dg = fbNeg || 'Cette source était fiable.'; }
              else { src.fb.tb = fbPos || 'Cette source n\'était pas fiable !'; src.fb.db = fbNeg || 'Bien joué de t\'en méfier !'; }
            }
      ...
      if(imported.length === 0){
        alert('❌ Aucune question valide trouvée dans le CSV.\n\nVérifie que ton fichier correspond bien au format exporté par FakeMètre (colonnes identiques).');
        return;
      }
      
      if(confirm(`Importer ${imported.length} question(s) depuis le CSV ? (Les questions existantes seront conservées)`)){
        customQuestions = [...customQuestions, ...imported];
        saveToStorage();
        updateQuestionCount();
        renderQuestionsList();
      ...
      alert('❌ Erreur lors de l\'import CSV : ' + err.message + '\n\nAssure-toi que le fichier est bien un export FakeMètre.');
    }
  };
  reader.readAsText(file);
  event.target.value = '';
  ...
  reader.onload = function(e){
    try {
      ...
      for(let i = 1; i < rows.length; i++){
        ...
        const question = { s: cols[0], t: cols[1].toUpperCase() === 'VRAI', e: cols[2], src: [] };
      ...
      if(imported.length === 0){
        alert('❌ Aucune question valide trouvée.\n\nAssure-toi que le fichier a bien été exporté depuis FakeMètre (CSV ou Excel).');
        return;
      }
      ...
      alert('❌ Erreur lors de l\'import : ' + err.message + '\n\nAssure-toi que le fichier a été exporté depuis FakeMètre.');
    }
  };
...
// ═══════════════════════════════════════════════
// UPLOAD DE FICHIERS (sources créateur)
// ═══════════════════════════════════════════════
...
async function uploadSourceFile(srcId, input){
  ...
  if(urlInput){ urlInput.value='⏳ Upload en cours...'; urlInput.disabled=true; }
...
// ═══════════════════════════════════════════════
// SUPABASE — COMMUNAUTÉ
// Remplace les deux valeurs ci-dessous par celles
...
function loadPacks(){
  ...
  grid.innerHTML='<div class="comm-loading">⏳ Chargement...</div>';
  if(SB_URL==='REMPLACER_PAR_TON_URL_SUPABASE'){
    grid.innerHTML='<div class="comm-error">⚙️ Supabase non configuré.<br><small>Suis les instructions dans le code source (SB_URL / SB_KEY).</small></div>';
    return;
  }
  ...
  sbFetch(q).then(data=>{
    ...
    grid.innerHTML='<div class="comm-error">❌ Impossible de charger les packs.<br><small>Vérifie ta connexion internet.</small></div>';
  });
}

function escH(s){return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}

const LEVEL_LABELS={junior:'Apprenti',inter:'Enquêteur',pro:'Fact-Checker',mixed:'Mixte'};
const LANG_FLAGS={fr:'🇫🇷',en:'🇬🇧',both:'🌍'};

function renderPacks(){
  const grid=$('pack-grid');
  if(!communityPacks.length){
    grid.innerHTML='<div class="comm-loading">Aucun pack pour l\'instant.<br>Sois le premier à publier !</div>';
    return;
  }
  ...
  grid.innerHTML=communityPacks.map(p=>{
    ...
    <div class="pack-card" id="pack-${p.id}">
      ...
      <div class="pack-meta">Par ${escH(p.author||'Anonyme')} · ${new Date(p.created_at).toLocaleDateString('fr-FR')}</div>
      ...
      <div class="pack-footer">
        ...
        <span class="pack-stat">🎮 ${p.play_count||0} parties</span>
      </div>
      <div class="pack-actions">
        ...
        <button class="comment-btn" onclick="toggleComments('${p.id}')">💬 Commentaires</button>
        <button class="comment-btn" onclick="editPack('${p.id}')">✏️ Modifier</button>
        ${isMine?`<button class="pack-delete-btn" onclick="deleteMyPack('${p.id}')">🗑️ Supprimer</button>`:''}
        ${adminMode?`<button class="pack-delete-btn" onclick="adminDeletePack('${p.id}')">🗑️ Admin</button>`:''}
        <button class="pack-play-btn" onclick="playCommunityPack('${p.id}')">Jouer ▶</button>
      </div>
      <div class="comments-section" id="comments-${p.id}" style="display:none;">
        <div id="comments-list-${p.id}" class="comments-list"></div>
        <div class="comment-form">
          <input type="text" id="comment-author-${p.id}" class="form-input" placeholder="Ton pseudo (optionnel)" maxlength="30">
          <textarea id="comment-content-${p.id}" class="form-input" placeholder="Écris ton avis sur ce pack..." maxlength="500" rows="2" style="resize:none;"></textarea>
          <button id="comment-submit-${p.id}" class="creator-btn primary" style="padding:9px 20px;font-size:.9rem;" onclick="submitComment('${p.id}')">Envoyer</button>
        </div>
      </div>
    </div>`;
  }).join('');
...
function loadComments(packId){
  ...
  container.innerHTML='<div style="color:var(--mu);font-size:.8rem;padding:8px 0;">Chargement...</div>';
  sbFetch('comments?pack_id=eq.'+packId+'&order=created_at.asc&limit=50')
    .then(data=>{
      if(!data||!data.length){
        container.innerHTML='<div style="color:var(--mu);font-size:.8rem;text-align:center;padding:12px 0;">Aucun commentaire. Sois le premier !</div>';
        return;
      }
      container.innerHTML=data.map(c=>`
        <div class="comment-item">
          <div class="comment-author">${escH(c.author||'Anonyme')}</div>
          <div class="comment-content">${escH(c.content)}</div>
          <div class="comment-date">${new Date(c.created_at).toLocaleDateString('fr-FR')}</div>
        </div>`).join('');
    }).catch(()=>{
      container.innerHTML='<div style="color:var(--rd);font-size:.8rem;">Erreur de chargement.</div>';
    });
}

function submitComment(packId){
  const content=document.getElementById('comment-content-'+packId).value.trim();
  const author=document.getElementById('comment-author-'+packId).value.trim()||'Anonyme';
  if(!content){alert('Écris un commentaire avant d\'envoyer !');return;}
...
function playCommunityPack(id){
  const pack=communityPacks.find(p=>p.id===id);
  if(!pack||!pack.questions||!pack.questions.length){alert('Pack invalide ou vide.');return;}
...
function openPublishModal(){
  if(!customQuestions.length){alert('Crée d\'abord des questions dans le Mode Créateur !');return;}
  $('pub-secret').value='';
  if(editingPackId&&editingPackData){
    ...
    $('publish-modal-title').textContent='✏️ Mettre à jour le pack';
    $('pub-submit-btn').textContent='💾 Mettre à jour';
    $('secret-hint-text').textContent='(entre ton code pour confirmer)';
    ...
    $('publish-modal-title').textContent='🌍 Publier sur la Communauté';
    $('pub-submit-btn').textContent='🚀 Publier !';
    $('secret-hint-text').textContent='(optionnel — nécessaire pour modifier ou supprimer plus tard)';
...
async function submitPublish(){
  ...
  if(!title){alert('Donne un titre à ton pack !');$('pub-title').focus();return;}
  if(SB_URL==='REMPLACER_PAR_TON_URL_SUPABASE'){alert('Supabase non configuré.');return;}
  const secretRaw=$('pub-secret').value.trim();
  const btn=$('pub-submit-btn');

  if(editingPackId){
    if(!secretRaw){alert('Entre ton code secret pour confirmer la mise à jour.');return;}
    const hash=await sha256(secretRaw);
    if(hash!==editingPackSecret){alert('❌ Code incorrect.');return;}
  ...
  if(!$('pub-charter-check').checked){alert('Tu dois accepter la charte de publication avant de continuer.');return;}
  ...
  sbFetch('packs',{
    ...
    body:JSON.stringify({
      title,
      description:$('pub-desc').value.trim(),
      author:$('pub-author').value.trim()||'Anonyme',
      level:$('pub-level').value,
      lang:$('pub-lang').value,
      secret_hash,
      questions:customQuestions
...
function showShareModal(url){
  ...
  m.innerHTML=`
    <div class="modal-box" style="max-width:440px;text-align:center;">
      ...
      <h3 style="margin-bottom:8px;">Pack publié !</h3>
      <p style="color:var(--mu);font-size:.85rem;font-weight:600;margin-bottom:16px;">Partage ce lien pour que les joueurs accèdent directement à ton set :</p>
      <div style="display:flex;gap:8px;align-items:center;">
        <input id="share-url-input" type="text" value="${url}" readonly class="form-input" style="font-size:.78rem;flex:1;">
        <button onclick="copyShareUrl()" class="creator-btn primary" style="padding:10px 14px;white-space:nowrap;">Copier</button>
      </div>
      <button class="creator-btn" style="margin-top:14px;width:100%;" onclick="document.getElementById('share-modal').remove()">Fermer</button>
    </div>`;
...
// ── Mes packs (propriété localStorage) ──
...
async function deleteMyPack(id){
  ...
  if(!hash){alert('Aucun code secret enregistré pour ce pack sur cet appareil.');return;}
  if(!confirm('Supprimer définitivement ton pack ?')) return;
  sbFetch('packs?id=eq.'+id+'&secret_hash=eq.'+hash,{method:'DELETE',prefer:'return=minimal'})
    .then(()=>{delete tokens[id];localStorage.setItem('fakemetre_my_packs',JSON.stringify(tokens));alert('Pack supprimé.');loadPacks();})
    .catch(e=>alert('❌ Erreur : '+e.message));
}

// ── Mode édition Creator ──
...
async function editPack(id){
  ...
  let hash=tokens[id];
  if(!hash){
    const code=prompt('Entre le code secret de ce pack :');
    if(code===null) return;
    if(!code.trim()){alert('Saisis un code secret.');return;}
  ...
  }catch(e){alert('❌ Erreur de connexion.');return;}
  if(!data||!data.length){alert('❌ Code incorrect.');return;}
  const pack=data[0];
  editingPackId=id;
  editingPackSecret=hash;
  editingPackData={title:pack.title,description:pack.description||'',author:pack.author||'',level:pack.level||'mixed',lang:pack.lang||'fr'};
  ...
  alert(`✅ Pack chargé (${customQuestions.length} question(s)). Modifie tes questions, puis clique sur "Mettre à jour" dans cet onglet.`);
}
...
let adminMode=false;
const ADMIN_HASH='a665a45920422f9d417e4867efdc4fb8a04a1f3fff1fa07e998e86f7f7a27ae3'; // sha256 de "123"
...
function adminDeletePack(id){
  if(!adminMode) return;
  if(!confirm('Supprimer définitivement ce pack ? (admin)')) return;
  sbFetch('packs?id=eq.'+id,{method:'DELETE',prefer:'return=minimal'})
    .then(()=>{ alert('Pack supprimé.'); loadPacks(); })
    .catch(e=>alert('❌ Erreur : '+e.message));
}
...
(function(){
  ...
  window.addEventListener('load',function(){
    ...
    sbFetch('packs?id=eq.'+packId+'&select=id,title,description,author,level,play_count,likes,created_at,questions&limit=1')
      .then(data=>{
        if(!data||!data.length){alert('Pack introuvable.');return;}
        const p=data[0];
      ...
      }).catch(()=>alert('Impossible de charger ce pack.'));
  });
})();
...
function _updFsBtn(){
  ...
  btn.innerHTML=isFs?'✕ Quitter':'⛶ Plein écran';
}
